...
//    __block MBProgressHUD *hud;
    BOOL willShowHud = [params[@"data"] count] <= 0;
    if (willShowHud) {
        dispatch_async(dispatch_get_main_queue(), ^{
            //
//            UIView *view_t = [MJTool getTopView];
//            if (view_t) {
//                hud = [MBProgressHUD showHUDAddedTo:view_t animated:YES];
//            }
        });
    }
//    hud.label.text = @"";//加载中...
...
- (AFHTTPSessionManager *)manager
{
    if(!_manager){
        ...
        NSURL *url;
        if (_baseURLString) {
            url = [NSURL URLWithString:_baseURLString];
        }
        ...
        _manager.responseSerializer.acceptableContentTypes = [NSSet setWithObjects:@"application/json", @"text/json", @"text/plain",@"text/javascript",@"text/html", @"image/gif", @"application/x-www-form-urlencoded", nil];//
